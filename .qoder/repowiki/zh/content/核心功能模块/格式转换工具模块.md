...
本模块聚焦于“格式转换工具”，当前版本实现了一个基于 Electron + Vue 的桌面应用中的 JSON 格式化功能。该功能允许用户将输入的 JSON 文本进行解析与美化输出，并提供基础的错误提示与清空结果的能力。尽管当前仅支持 JSON 格式化，但整体架构已为后续扩展至多格式转换（如 XML、CSV、YAML 等）预留了清晰的扩展点。
...
- 格式转换视图：实现 JSON 输入、解析与美化输出，包含错误处理与消息提示
